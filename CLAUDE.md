...
### Feedback round: auto-advance robustness + guest UX

Owner testing surfaced four items, all addressed:

- **Auto-advance did not reliably start the next song.** The mechanism (TV posts `party_song_ended` on `isFinished` -> server advances) is correct and works with a live, current TV tab, but it made the TV a single point of failure: if the TV tab is closed, backgrounded, or on a stale bundle, `song_ended` never arrives and the entry stays `Playing`, stalling the queue. **Fix: a server-side duration watchdog** (`spawn_advance_watchdog` in `queue_service.rs`). When a song starts, a task waits its duration + 6s grace; if it is still the current song (same `play_token`) and not paused, it advances anyway. The TV report is still the prompt path; the watchdog is the safety net. It is superseded by a `play_token` change (skip / next) and waits while paused, so it never cuts a song short. Needs tokio's `time` feature. Tested by `scripts/party-test/t3c-watchdog.mjs`, which queues two songs, sends **no** `song_ended`, and asserts the watchdog advances.
- **Online search had too many steps.** Collapsed from four interactions (type -> "Search online" button -> pick LRCLIB match -> pick video) to two (type a not-in-library song -> pick a video). `OnlineSearch` now runs automatically when the local search is empty, does the LRCLIB lookup itself (top lyric match supplies the canonical metadata), and lists YouTube videos directly, with the matched `Artist - Title` shown as a header.
- **Username entry.** Removed the blocking full-screen name gate; the party page now has an always-visible inline "Your name" field (persisted to `localStorage` as typed) that attributes every add. The gate had been invisible once a name was stored, which is why it seemed missing.
- **Guest queue control.** The `/party` queue was read-only; it now has per-entry reorder (up/down, computed against the full array so indices stay correct), remove, and a Skip button on the now-playing entry (reusing `party_skip` / `party_queue_remove` / `party_queue_reorder`). Consistent with the no-auth LAN trust model: any guest can steer the queue.
